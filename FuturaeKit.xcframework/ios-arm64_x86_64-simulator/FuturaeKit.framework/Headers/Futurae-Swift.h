...
@class FTRExtraInfo;
SWIFT_ENUM_FWD_DECL(NSInteger, AuthReplyType)
...
/// Creates an <code>AuthReplyParameters</code> instance for a mobile auth authentication request using session token.
/// note:
/// Only <code>.approve</code> is a valid reply type for mobile auth. Passing <code>.reject</code> or <code>.fraud</code> will result in an <code>SDKError</code> with code <code>.invalidParameter</code>.
/// \param reply The type of reply. Only <code>.approve</code> is supported.
///
/// \param sessionToken The session token for the authentication request.
///
/// \param userId The user identifier.
...
/// warning:
/// Deprecated. Rejecting a mobile auth request is not supported. Passing the resulting parameters to <code>replyAuth</code> will result in an <code>SDKError</code> with code <code>.invalidParameter</code> being thrown.
/// \param sessionToken The session token for the authentication request.
///
/// \param userId The user identifier.
///
...
+ (RejectMobileAuth * _Nonnull)rejectMobileAuthWithSessionToken:(NSString * _Nonnull)sessionToken userId:(NSString * _Nonnull)userId isFraud:(BOOL)isFraud extraInfo:(NSArray<FTRExtraInfo *> * _Nullable)extraInfo SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_MSG("Rejecting a mobile auth request is not supported. Passing the resulting parameters to replyAuth will result in an SDKError with code .invalidParameter.");
...
/// <code>EnrollmentExchangeTokensParameters</code> encapsulates the inputs required to
/// fetch a batch of short-lived enrollment Exchange Tokens (rendered as QR
/// codes by the host app).
SWIFT_CLASS("_TtC10FuturaeKit34EnrollmentExchangeTokensParameters")
@interface EnrollmentExchangeTokensParameters : NSObject
/// Short activation code returned by <code>spawnEnrollment(...)</code>.
@property (nonatomic, readonly, copy) NSString * _Nonnull activationCodeShort;
/// Total number of Exchange Tokens to generate in this batch.
/// Server-side bounds: min <code>1</code>, max <code>50</code>.
@property (nonatomic, readonly) NSInteger totalExchangeTokens;
/// Minimum lifetime of a single Exchange Token (QR), in seconds.
/// Typically <code>APP_2_APP_QR_TOTAL_LIFETIME - APP_2_APP_QR_OVERLAP</code>.
@property (nonatomic, readonly) NSInteger exchangeTokenLifetimeSeconds;
- (nonnull instancetype)initWithActivationCodeShort:(NSString * _Nonnull)activationCodeShort totalExchangeTokens:(NSInteger)totalExchangeTokens exchangeTokenLifetimeSeconds:(NSInteger)exchangeTokenLifetimeSeconds OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
SWIFT_ENUM_FWD_DECL(NSInteger, FTRMessageLevel)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKStatus)
...
/// warning:
/// Deprecated. Integrity checks should be implemented directly in the host app using Apple’s App Attest. See <a href="https://developer.apple.com/documentation/devicecheck/establishing-your-app-s-integrity">Establishing your app’s integrity</a>.
...
///
- (void)appAttestationWithAppId:(NSString * _Nonnull)appId production:(BOOL)production success:(void (^ _Nonnull)(void))success failure:(void (^ _Nonnull)(NSError * _Nonnull))failure SWIFT_DEPRECATED_MSG("Implement integrity checks directly in the host app using Apple's App Attest: https://developer.apple.com/documentation/devicecheck/establishing-your-app-s-integrity") SWIFT_AVAILABILITY(ios,introduced=14.0);
@end
...
/// Upon successful retrieval, the <code>success</code> closure is called  containing the enrollment activation short code. In case of failure during the retrieval process, the <code>failure</code> closure is executed with an error providing details about the failure reason.
/// \param exchangeToken A valid exchange token retrieved from a QR code or URL.
///
/// \param success A closure to be called upon successful retrieval of the enrollment activation short code.
///
...
SWIFT_ENUM_FWD_DECL(NSInteger, FTRQRCodeType)
...
@class SpawnEnrollmentParameters;
@class FTRSpawnedEnrollment;
@class FTREnrollmentExchangeTokenQRCode;
@interface FTRClient (SWIFT_EXTENSION(FuturaeKit))
/// Spawns a new App-2-App enrollment for an already-enrolled user.
/// On success the SDK returns an <code>FTRSpawnedEnrollment</code> containing the
/// short activation code and enrollment id. The activation short code is
/// then used with <code>getEnrollmentExchangeTokens(_:success:failure:)</code> to
/// obtain a batch of short-lived Exchange Tokens, which the host app
/// renders as QR codes for a receiving device.
/// This is a protected operation: the SDK must be unlocked.
/// \param parameters Inputs for the spawn operation. See <code>SpawnEnrollmentParameters</code>.
///
/// \param success Closure invoked with the spawned enrollment details.
///
/// \param failure Closure invoked with an error if the operation fails, including
/// <code>SDKError.invalidParameter</code> when <code>validDurationSeconds</code> is out of range.
///
- (void)spawnEnrollment:(SpawnEnrollmentParameters * _Nonnull)parameters success:(void (^ _Nonnull)(FTRSpawnedEnrollment * _Nonnull))success failure:(void (^ _Nonnull)(NSError * _Nonnull))failure;
/// Fetches a batch of short-lived enrollment Exchange Tokens.
/// Each token in the batch is intended to be rendered as a QR code by the
/// host app, always displaying the one with the closest expiration first.
/// This is a protected operation: the SDK must be unlocked.
/// \param parameters Inputs for the operation. See <code>EnrollmentExchangeTokensParameters</code>.
///
/// \param success Closure invoked with the array of Exchange Token QR payloads.
///
/// \param failure Closure invoked with an error if the operation fails.
///
- (void)getEnrollmentExchangeTokens:(EnrollmentExchangeTokensParameters * _Nonnull)parameters success:(void (^ _Nonnull)(NSArray<FTREnrollmentExchangeTokenQRCode *> * _Nonnull))success failure:(void (^ _Nonnull)(NSError * _Nonnull))failure;
@end
...
///
/// returns:
/// the release-candidate number for RC builds, or nil for beta/stable builds.
@property (nonatomic, readonly, strong) NSNumber * _Nullable releaseCandidate;
@end
...
/// <code>FTREnrollmentExchangeTokenQRCode</code> represents a single short-lived Exchange
/// Token returned by <code>getEnrollmentExchangeTokens(_:success:failure:)</code>, intended
/// to be rendered as a QR code by the host app during an App-2-App enrollment.
SWIFT_CLASS("_TtC10FuturaeKit32FTREnrollmentExchangeTokenQRCode")
@interface FTREnrollmentExchangeTokenQRCode : NSObject
/// The QR code content to render as an image.
@property (nonatomic, readonly, copy) NSString * _Nonnull qrCode;
/// Unix timestamp (in seconds) at which this Exchange Token expires.
@property (nonatomic, readonly) NSInteger exp;
- (nonnull instancetype)initWithQrCode:(NSString * _Nonnull)qrCode exp:(NSInteger)exp OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
SWIFT_ENUM_FWD_DECL(NSInteger, FTRKeychainItemAccessibility)
...
/// <code>FTRSpawnedEnrollment</code> holds the result of spawning an App-2-App enrollment
/// via <code>spawnEnrollment(_:success:failure:)</code>.
SWIFT_CLASS("_TtC10FuturaeKit20FTRSpawnedEnrollment")
@interface FTRSpawnedEnrollment : NSObject
/// Short activation code for the spawned enrollment. Pass it to
/// <code>getEnrollmentExchangeTokens(_:success:failure:)</code> to obtain the
/// short-lived Exchange Token QR codes.
@property (nonatomic, readonly, copy) NSString * _Nonnull activationCodeShort;
/// Identifier of the spawned enrollment.
@property (nonatomic, readonly, copy) NSString * _Nonnull enrollmentId;
- (nonnull instancetype)initWithActivationCodeShort:(NSString * _Nonnull)activationCodeShort enrollmentId:(NSString * _Nonnull)enrollmentId OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
SWIFT_ENUM_FWD_DECL(NSInteger, UserPresenceVerificationType)
...
SWIFT_ENUM_FWD_DECL(NSInteger, NotificationPayloadDataType)
...
SWIFT_ENUM_FWD_DECL(NSInteger, LockConfigurationType)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKErrorType)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKAccountsErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKApiErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKAppAttestErrorCode)
...
/// The App Attest operation did not complete within the allotted time.
/// Apple’s <code>DCAppAttestService</code> can stall (throttling, connectivity), so the SDK bounds the call
/// and surfaces this instead of hanging indefinitely.
  SDKAppAttestErrorCodeTimeout = 9,
};
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKBiometricsStateErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKConnectionErrorCode)
@class NSURLResponse;
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKCryptoErrorCode)
...
/// Failed to generate random number for encryption
  SDKCryptoErrorCodeSecRandomNumberFailed = 15,
};

SWIFT_ENUM_FWD_DECL(NSInteger, SDKDatabaseErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKErrorCode)
...
/// An invalid parameter was provided.
  SDKErrorCodeInvalidParameter = 31,
};
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKLockErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKMigrationErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKPinErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKSwitchLockErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKSystemErrorCode)
...
SWIFT_ENUM_FWD_DECL(NSInteger, SDKURLErrorCode)
...
/// <code>SpawnEnrollmentParameters</code> encapsulates the inputs required to start an
/// App-2-App enrollment for an already-enrolled user.
SWIFT_CLASS("_TtC10FuturaeKit25SpawnEnrollmentParameters")
@interface SpawnEnrollmentParameters : NSObject
/// The user identifier of an account already enrolled on this device,
/// for which a new enrollment will be created.
@property (nonatomic, readonly, copy) NSString * _Nonnull userId;
/// Validity duration of the spawned enrollment, in seconds. Must be within
/// [<code>minValidDurationSeconds</code>, <code>maxValidDurationSeconds</code>]; <code>spawnEnrollment</code>
/// fails with <code>SDKError.invalidParameter</code> otherwise. Defaults to
/// <code>minValidDurationSeconds</code> (60) when not specified.
@property (nonatomic, readonly) NSInteger validDurationSeconds;
- (nonnull instancetype)initWithUserId:(NSString * _Nonnull)userId;
- (nonnull instancetype)initWithUserId:(NSString * _Nonnull)userId validDurationSeconds:(NSInteger)validDurationSeconds OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
