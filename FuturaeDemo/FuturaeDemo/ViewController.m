...
- (IBAction)logoutTouchedUpInside:(UIButton *)sender
{
    ...
    NSDictionary *account = [[FuturaeClient sharedClient] getAccounts].firstObject;
    if (account == nil) {
...
{
    NSDictionary *account = [[FuturaeClient sharedClient] getAccounts].firstObject;
    NSDictionary *totp = [[FuturaeClient sharedClient] nextTotpForUser:account[@"user_id"]];
...
- (void)reader:(FTRQRCodeViewController * _Nullable)reader didScanResult:(NSString *_Nullable)result
{
    ...
    if (comps.count == 2) {
        ...
        [[FuturaeClient sharedClient] approveAuthWithQrCode:result
